...
class BlankOBFv2:
    ...
    def __init__(self, code: str, include_imports: bool=False, recursion: int=1) -> None:
        ...
        self._imports = []
        self._aliases = {}
        self._valid_identifiers = [chr(i) for i in range(256, 149878) if chr(i).isidentifier()]
    ...
    def obfuscate(self) -> str:
        ...
        self._save_imports()
        layers = [self._layer_1, self._layer_2, self._layer_3] * self.__recursion
        ...
        class Transformer(ast.NodeTransformer):
            ...
            def visit_Name(self, node: ast.Name) -> ast.Name:
                if node.id in dir(builtins) or node.id in [x[1] for x in self._outer._imports]:
                    node = ast.Call(func=ast.Call(func=ast.Name(id='getattr', ctx=ast.Load()), args=[ast.Call(func=ast.Name(id='__import__', ctx=ast.Load()), args=[self.visit_Constant(ast.Constant(value='builtins'))], keywords=[]), self.visit_Constant(ast.Constant(value='eval'))], keywords=[]), args=[ast.Call(func=ast.Name(id='bytes', ctx=ast.Load()), args=[ast.Subscript(value=ast.List(elts=[ast.Constant(value=x) for x in list(node.id.encode())][::-1], ctx=ast.Load()), slice=ast.Slice(upper=None, lower=None, step=ast.Constant(value=-1)))], keywords=[])], keywords=[])
                    return node
                else:
                ...
                if isinstance(node.value, int):
                    ...
                    match choice:
                        ...
                        case 2:
                            ...
                            node = ast.BinOp(left=ast.BinOp(left=ast.BinOp(left=ast.BinOp(left=ast.Constant(value=num * 2), op=ast.Add(), right=ast.Constant(value=node.value * 2 * times)), op=ast.FloorDiv(), right=ast.Constant(value=2)), op=ast.Sub(), right=ast.Constant(value=num)), op=ast.Sub(), right=ast.Constant(node.value * (times - 1)))
                elif isinstance(node.value, str):
                    encoded = list(node.value.encode())[::-1]
                    node = ast.Call(func=ast.Attribute(value=ast.Call(func=ast.Name(id='bytes', ctx=ast.Load()), args=[ast.Subscript(value=ast.List(elts=[ast.Constant(value=x) for x in encoded], ctx=ast.Load()), slice=ast.Slice(lower=None, upper=None, step=ast.Constant(value=-1)), ctx=ast.Load())], keywords=[]), attr='decode', ctx=ast.Load()), args=[], keywords=[])
                elif isinstance(node.value, bytes):
                    encoded = list(node.value)[::-1]
                    node = ast.Call(func=ast.Name(id='bytes', ctx=ast.Load()), args=[ast.Subscript(value=ast.List(elts=[ast.Constant(value=x) for x in encoded], ctx=ast.Load()), slice=ast.Slice(lower=None, upper=None, step=ast.Constant(value=-1)), ctx=ast.Load())], keywords=[])
                return node

            def visit_Attribute(self, node: ast.Attribute) -> ast.Attribute:
                node = ast.Call(func=ast.Name(id='getattr', ctx=ast.Load()), args=[node.value, ast.Constant(node.attr)], keywords=[])
                return self.generic_visit(node)
        tree = ast.parse(self._code)
    ...
    def _layer_1(self) -> None:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign) and isinstance(node.value, ast.Constant) and isinstance(node.value.value, str) and parts:
                ...
                part = parts.pop()
                node.value = ast.Subscript(value=ast.Constant(value=before + part + after), slice=ast.Slice(upper=ast.Constant(value=len(before) + len(part)), lower=ast.Constant(value=len(before)), step=None), ctx=ast.Store())
        self._code = ast.unparse(tree)
...
def main() -> None:
    parser = argparse.ArgumentParser(prog=os.path.basename(__file__), description='BlankOBF v2: Obfuscates Python code to make it unreadable and hard to reverse.')
    ...
    parser.add_argument('--recursive', required=False, help='Recursively obfuscates the code N times (slows down the code; not recommended)', metavar='N')
    parser.add_argument('--include_imports', required=False, action='store_true', help='Include the import statements on the top of the obfuscated file')
